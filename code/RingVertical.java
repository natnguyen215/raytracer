 ...
 * @version 2022.05.11
 */
public class RingVertical extends Surface {
    ...
    public RingVertical(Point position, double outerRadius, double innerRadius, Vector norm, Material m){
        r1 = outerRadius;
        r2 = innerRadius;
        normal = norm;
        mat = m;
        ...
        if (distance>EPSILON){
            ...
            if (length < r1 && length > r2){
                if(normal.dot(ray.getDirection()) > 0){
                    ...
                    return new Intersection(inter, normal.scale(-1), distance, mat, -(inter.getX()-center.getX() - (r1))/r1/2, -(inter.getY()-center.getY() - (r1))/r1/2);
                }
                else {
                    //System.out.println(inter.getX());
                    //System.out.println("hi");
                    //System.out.println(center.getX());
                    return new Intersection(inter, normal, distance, mat, -(inter.getX()-center.getX() - (r1))/r1/2, -(inter.getY()-center.getY() - (r1))/r1/2);
                }
            }
        }
        return null;
